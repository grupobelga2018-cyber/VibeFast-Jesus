...
function serviceWhen(appointment, startsAt = appointment.starts_at) {
  const serviceName = displayServiceName(appointment)
...
export async function sendAppointmentConfirmation(
  ...
  const { serviceName, when } = serviceWhen(appointment)
...
export async function sendWaitingForGaby(appointment, { rescheduled = false } = {}) {
  ...
  const { serviceName, when } = serviceWhen(appointment, starts)
  const text = [
    rescheduled
...
export async function sendRescheduleRejected(appointment) {
  ...
  return sendTelegramMessage(
    ...
    [
      ...
      `Tu cita sigue: ${serviceName} · ${when}`,
